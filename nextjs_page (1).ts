...
export default function StructCalcHome() {
  ...
  const updateElement = (id: number, field: keyof Element, value: string | number) => {
  ...
  const generateReport = () => {
    let report = "REPORTE ESTRUCTURA METÁLICA - StructCalc\n\n";
    ...
    report += `\nRESUMEN:\n`;
    report += `Total elementos: ${elements.length}\n`;
    report += `Peso total: ${totalWeight.toFixed(1)} kg\n`;
    report += `Precio por kg: $${steelPrice.toLocaleString('es-MX')}\n`;
    report += `Costo estimado: $${totalCost.toLocaleString('es-MX')}\n\n`;
    report += `Generado por StructCalc - ${new Date().toLocaleDateString('es-MX')}\n`;
    report += `www.structcalc.mx\n`;
    
    const blob = new Blob([report], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `estructura_${new Date().getTime()}.txt`;
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-500">Calculadora profesional de peso para estructuras metálicas</p>
          <div className="mt-4 text-sm text-gray-400">
            Beta v1.0 - Perfiles estándar mexicanos
          </div>
        </div>
        ...
        {elements.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 mb-6">Agrega elementos metálicos para calcular el peso total de tu estructura</p>
            <button 
              onClick={addElement}
        ...
        <div className="mt-16 pt-8 border-t border-gray-100 text-center text-sm text-gray-400">
          <p>© 2024 StructCalc - Calculadora profesional para estructuras metálicas</p>
          <p className="mt-2">Desarrollado en México 🇲🇽</p>
        </div>
